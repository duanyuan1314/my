...
class Run extends Admin
{
    ...
    public function index()
    {
        ...
        if (!empty($request['starttime']) && !empty($request['endtime'])) 
        {
            ...
            $map['c.creat_time'] = array(array('>=',$keyword1),array('<=',$keyword2));
        ...
        $count = DB::name('purse_log')
                ->alias('c')
                ->join('hisi_user u','u.id=c.uid')
                ->field('sum(amout) as amout,count(c.id) as sumpeo')
                ->where($map)->select();
        $peocount = DB::name('purse_log')
                ->alias('c')
                ->join('hisi_user u','u.id=c.uid')
    ...
    {
            $id  = input('param.id');
            $res = DB::name('purse_log')->where('id', $id)->setField('status', 2);
            if ($res === false) {
    ...
    {
            $id  = input('param.id');
            $res = DB::name('purse_log')->where('id', $id)->setField('status', 1);
            if ($res === false) {
                return $this->error('操作失败！');
            }
            return $this->success('应用成功');
