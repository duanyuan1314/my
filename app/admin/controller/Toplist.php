...
class Toplist extends Admin
{
    ...
    */
    ...
    {
        $url = 'http://playgame.hnlfcywlkj.com/Home/Api/toplist';
   
        $list = post_https($url,3); //

        $data_list = json_decode($list,true);//->toArray();
        if($data_list['code']==400){
            return $this->error('温馨提示,暂无数据');
        }else{
            Db::query('truncate hisi_toplist;');
            foreach ($data_list['data'] as $key => $value) {
                Db::name('toplist')->insert($value);
            }
            $this->redirect('admin/toplist/index');
        }
    ...
    public function index($q = '')
    {
        ...
        $data_list = Db::name('toplist')->where($sqlmap)->order('id desc')->paginate('10');
        // 分页
    ...
    public function deltop()
    {
        ...
        //$model = new Brand();
        $flag = Db::name('toplist')->delete($ids);
        if ($flag) {
    ...
    **/
    public function brand()
    {
        ...
        $map=array();
        $data_list = Brand::where($map)->paginate();
        $pages = $data_list->render();
        $tab_data = $this->tab_data;
        $tab_data['current'] = url('');
        $this->assign('role_list', RoleModel::getAll());
