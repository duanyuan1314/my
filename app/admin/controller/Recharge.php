...
use app\admin\model\Recharge as RechModel;
use think\Validate;
use think\Db;
...
class Recharge extends Admin
{
    ...
    public function index($q = '')
    {
        ...
        $data_list = RechModel::where($sqlmap)->paginate();
        //分页
    ...
    *称号编辑
    */
    public function rechedit($id)
    {

        if ($this->request->isPost()) {
            $data = $this->request->post();

            if (!RechModel::update($data)) {
                return $this->error('修改失败');
            }
        ...
        $row = RechModel::where('id', $id)->field('*')->find()->toArray();
    ...
    */
    public function rechadd()
    {
        if ($this->request->isPost()) {
            ...
            //$data['ishidden'] = 0;
            $data['addtime'] = time();
            $ary = session('admin_user');
            $data['usernick'] = $ary['nick'];
            //var_dump($data);
            if (!RechModel::create($data)) {
                return $this->error('添加失败');
            }
            return $this->success('添加成功');
        }
